          ...
          {demoSections.map((demo) =>
            demo.Component ? (
              <motion.div
                key={demo.id}
                id={`demo-${demo.id}`}
                className="border border-gray-200 rounded-lg p-4 md:p-6 shadow-sm flex flex-col bg-white"
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.3 }}
                transition={{ duration: 0.5 }}
              >
                <h3 className="text-xl md:text-2xl font-semibold text-raven-dark mb-2">
                  {demo.title}
                </h3>
                <p className="text-gray-600 mb-4 text-base">{demo.desc}</p>
              ...
              <div
                key={demo.id}
                id={`demo-${demo.id}`}
                className="border border-dashed border-gray-300 rounded-lg p-4 md:p-6 text-center text-gray-500"
              >
                <h3 className="text-xl md:text-2xl font-semibold text-raven-dark mb-2">
                  {demo.title}
                </h3>
                <p className="mb-4">{demo.desc}</p>
